...
interface User {
    ...
    address: string;
    city: string;
    createdAt: string; // Puoi convertirlo a Date se necessario
    dateOfBirth: string; // Puoi convertirlo a Date se necessario
    email: string;
    fullName: string;
    phoneNumber: string;
    province: string;
...
interface Product {
    ...
    sold: boolean;
    userId: string;
    user?: User; // Aggiungiamo l'utente che ha pubblicato il prodotto
}
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                // Query Firestore per prodotti non venduti nella categoria "Arredamento"
                const q = query(
                ...
                for (const docSnap of querySnapshot.docs) {
                    ...
                    if (productData.userId !== user.uid) {
                        // Recupera l'utente che ha pubblicato il prodotto
